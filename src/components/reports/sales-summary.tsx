...
import { fetchSalesSummary } from '@/lib/firebase/reports';
...
export default function SalesSummaryReport() {
  ...
  const handleDateChange = async (startDate: Date | undefined, endDate: Date | undefined) => {
    try {
      setLoading(true);
      const salesData = await fetchSalesSummary(startDate, endDate);
      setData(salesData);
    } catch (error) {
    ...
    try {
      ...
      const exportData = data.map(row => {
        ...
        visibleColumns.forEach(col => {
          filteredRow[columns.find(c => c.key === col)?.label || col] = row[col as keyof SalesSummaryData];
        });
        return filteredRow;
      });
      ...
      if (type === 'excel') {
        ...
        link.download = `sales_summary_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
      }
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={visibleColumns.length} className="text-center py-4">
                    No data available. Select a date range to view sales summary.
                  </td>
                </tr>
              ) : (
                data.map((row, index) => (
                      ...
                      visibleColumns.includes(column.key) && (
                        <td key={column.key} className="px-4 py-2">
                          {column.key === 'totalAmount' 
                            ...
                            : row[column.key as keyof SalesSummaryData]}
                        </td>
                      )
                    ))}
                  </tr>
